...
)
{
    ...
    case DISCONNECT_SERIAL:
    {
        ...
        */
        ...
        // empty queue
        for (int i = 0; i < (int)g_queue.size(); i++) g_queue.pop();
...
BOOL receive(LPVOID lpVoid)
{
    ...
    // if for whatever reason the receiver has to wait
    // another read is performed by setting the g_bSwitch to FALSE
    g_bSwitch = FALSE;
    g_cWait++;
    // when the receiver is finished the g_bSwitch goes to TRUE
    if (g_cWait >= 5) g_bSwitch = TRUE;
    return EXIT_SUCCESS;
}
